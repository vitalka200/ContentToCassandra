...
public interface ContentAccessor {
	...
	@Query("SELECT * FROM chunks WHERE url_id=:uuid AND chunk_seq_id=:seq_num")
	public Result<Chunk> getUrlChunkByIndex(@Param("uuid") UUID url_uuid, @Param("seq_num") int seq_num);
	
	@Query("SELECT * FROM chunks WHERE url_id=:uuid")
	...
	@Query("SELECT * FROM urls WHERE url_addr=:addr")
